...
async function loadEarningsTab(){
  ...
  for(let i=0;i<watchlist.length;i++){
    ...
    try{
      ...
      let impliedMove=null;try{const nearEntry=_nearestExpEntry(t);if(nearEntry&&snap.price){const atmP=_expPuts(nearEntry).filter(p=>Math.abs(p.strike-snap.price)/snap.price<0.03);const atmC=_expCalls(nearEntry).filter(c=>Math.abs(c.strike-snap.price)/snap.price<0.03);if(atmP.length&&atmC.length){const straddle=((atmP[0].bid+atmP[0].ask)/2)+((atmC[0].bid+atmC[0].ask)/2);impliedMove=(straddle/snap.price*100).toFixed(1);}}}catch{}
      const daysUntil=du; // already computed above via daysUntilDate
      earningsAllData.push({ticker:t,snap,earningsDate:_effEarningsDate,earningsHour:_effEarningsHour,daysUntil,epsEst,epsEstEndDate,epsActualPrev,surprisePrev,surpriseDollarPrev,beatStreak,missStreak,ivrVal,ivrBadge:ivr.badge,impliedMove,freshnessLabel,news:news.slice(0,3)});
    }catch{}
