...
html,body{background:var(--bg);color:var(--text);font-family:var(--sans);font-size:var(--base-font);min-height:100vh;overscroll-behavior:none}
#app{display:flex;flex-direction:column;min-height:100vh}
/* Market status banner */
#market-status-banner{font-family:var(--mono);font-size:12px;font-weight:600;padding:6px 16px;text-align:center;position:sticky;top:0;z-index:200;transition:background 0.5s}
.mkt-open{background:rgba(0,200,150,0.9);color:#000}
.mkt-prepost{background:rgba(255,165,2,0.9);color:#000}
.mkt-closed{background:rgba(255,71,87,0.85);color:#fff}
/* Header */
#top-progress-bar{position:fixed;top:0;left:0;right:0;height:3px;z-index:9999;background:transparent;pointer-events:none}
#top-progress-bar .top-bar-fill{height:3px;width:0%;background:linear-gradient(90deg,#00d4aa,#4fc3f7);transition:width 0.3s ease;border-radius:0 2px 2px 0}
#top-progress-bar.active .top-bar-fill{box-shadow:0 0 8px rgba(0,212,170,0.6)}
.header{background:var(--surface);border-bottom:1px solid var(--border);padding:10px 16px 8px;position:sticky;top:36px;z-index:100;display:flex;align-items:center;justify-content:space-between}
.header-left{display:flex;flex-direction:column;gap:2px}
.header-title{font-family:var(--sans);font-size:18px;font-weight:800;letter-spacing:-0.5px;color:var(--accent)}
.header-sub{font-family:var(--mono);font-size:10px;color:var(--text3);display:flex;align-items:center;gap:6px}
.online-dot{width:6px;height:6px;border-radius:50%;display:inline-block;flex-shrink:0}
...
#vix-status-banner{display:none;font-family:var(--mono);font-size:11px;font-weight:600;padding:3px 8px;border-radius:6px;margin-top:3px;width:fit-content}
...
/* Nav tabs */
.nav-tabs{display:flex;background:var(--surface);border-bottom:1px solid var(--border);overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch;position:sticky;top:94px;z-index:99}
.nav-tabs::-webkit-scrollbar{display:none}
.nav-tab{flex:0 0 auto;padding:10px 13px;font-family:var(--mono);font-size:11px;color:var(--text3);border:none;background:none;cursor:pointer;border-bottom:2px solid transparent;white-space:nowrap;transition:all 0.2s}
...
#offline-mode-bar{display:none;background:rgba(124,106,247,0.15);border-bottom:1px solid rgba(124,106,247,0.3);font-family:var(--mono);font-size:10px;color:#b39ddb;padding:4px 16px;text-align:center}
/* Layout */
...
.settings-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:300;backdrop-filter:blur(4px)}
...
.modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:400;align-items:center;justify-content:center;padding:20px}
.modal-overlay.open{display:flex}
.modal-box{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;max-width:340px;width:100%}
.modal-title{font-family:var(--sans);font-size:16px;font-weight:700;color:var(--danger);margin-bottom:12px}
.modal-body{font-family:var(--mono);font-size:12px;color:var(--text2);line-height:1.6;margin-bottom:16px}
...
.oi-empty-box{background:rgba(255,165,2,0.08);border:1px solid rgba(255,165,2,0.3);border-radius:var(--radius);padding:12px;font-family:var(--mono);font-size:11px;color:var(--warn);line-height:1.6;margin-top:8px}
...
<div class="header">
  <div class="header-left">
    ...
    <div class="header-sub">
      ...
      <span>Options Income Strategy v2.3.0</span>
    </div>
...
VIX backwardation (VIX above VIX3M): historically strong time to sell puts</div>
  ...
  <div class="card"><div class="card-title"><span class="dot"></span>About</div>
    <div class="commentary">Data: Finnhub (quotes, fundamentals, earnings, news, analyst ratings)
     Yahoo Finance via Cloudflare Worker (history, options, VIX, indices)
     FRED API (T-bill yields -- no key required)
...
<div class="settings-overlay" id="settings-overlay" onclick="closeSettingsIfOutside(event)">
  <div class="settings-sheet">
    ...
    <div class="input-group"><label class="input-label">Font Size</label><select class="input" id="font-size-input"><option value="15">Small (15px)</option><option value="17">Medium (17px)</option><option value="19" selected>Default (19px)</option><option value="21">Large (21px)</option><option value="23">Extra Large (23px)</option></select></div>
    ...
    <div class="commentary" style="font-size:12px">v2.3.0 -- 2026-04-29
...
const S={
  ...
  set:(k,v)=>{try{localStorage.setItem(k,JSON.stringify(v));}catch(e){if(e.name==='QuotaExceededError'||e.code===22){console.warn('Storage full -- could not save',k);toast('Storage full -- clear cached data in Settings',4000);}}}  ,
...
// NYSE holidays 2025-2026 (hardcoded, update annually)
...
function nowPT(){return nowInTZ();}// alias kept for compatibility
...
// Convert a fixed ET time (hour, min) to selected display timezone label
...
// ═══ OPTIONS PREFS -- mode-specific persistence ═══
...
async function fetchPriceTarget(symbol){
  // Yahoo quoteSummary financialData module -- contains price targets
  // Separate from the quote endpoint which does NOT include targetMeanPrice
  if(offlineMode)return null;
  try{
    const r=await fetch(`${WORKER_URL}/?ticker=${encodeURIComponent(symbol)}&type=summary&modules=financialData`);
    if(!r.ok)return null;
    const d=await r.json();
    ...
    return{
      ...
      ptMedian:fd.targetMedianPrice?.raw||null
    };
  }catch{return null;}
}
...
function getSortedWatchlist(){
  ...
  if(watchlistSort==='opportunity'){
    ...
    // Merge scores -- take max of puts and calls score per ticker
    const scoreMap={};
    [...puts,...cc].forEach(r=>{
...
async function loadTicker(){
  ...
  try{
    let snap,hist6mo,hist1y,news,recData,isLive=true,priceTargetData=null;
    try{
      ...
      try{priceTargetData=await fh(`/stock/price-target?symbol=${t}`);}catch{}
      ...
      S.set('snap_'+t,snap);S.set('rec_'+t,{data:(typeof rec2!=='undefined'&&rec2&&rec2.length?rec2[0]:recData)||null,ts:nowPT()});S.set('upgrades_'+t,{data:(typeof upgrades2!=='undefined'&&upgrades2&&upgrades2.length?upgrades2.slice(0,6):upgradeData)||[],ts:nowPT()});
      ...
      // Fetch price targets from Yahoo quoteSummary (separate endpoint from quote)
      try{
        const pt=await fetchPriceTarget(t);
        if(pt&&pt.ptMean){snap.ptMean=pt.ptMean;snap.ptHigh=pt.ptHigh||null;snap.ptLow=pt.ptLow||null;snap.ptAnalysts=pt.ptAnalysts||null;}
      }catch{}
    }catch{
    ...
    renderTickerContent(snap,hist6mo,hist1y,news,recData,upgradesData,isLive);renderWatchlist();
...
function restoreTickerFromCache(t){
  if(!t)return;const snap=S.get('snap_'+t);if(!snap)return;
...
function renderTickerContent(snap,hist,hist1y,news,recData,upgradesData,isLive){
  ...
  // Short interest: Finnhub provides shortInterest (shares) and shortRatio (days-to-cover)
  // Updated twice monthly; may be null on free tier for some tickers
  let shortStr='N/A (not reported)';
  if(snap.shortInterest&&snap.shortInterest>0){
  ...
  el.innerHTML=`<div class="card">
    ...
    ${(snap.postMarketPrice&&snap.postMarketPrice!==snap.price)?`<div style="font-family:var(--mono);font-size:12px;color:${snap.postMarketPrice>snap.price?'var(--green)':'var(--red)'};margin-bottom:8px">${snap.marketState==='PRE'?'Pre-market':'After-hours'}: $${snap.postMarketPrice.toFixed(2)}${snap.postMarketChange?` <span style="font-size:11px">${snap.postMarketChange>=0?'+':''}${snap.postMarketChange.toFixed(2)} (${snap.postMarketChange>=0?'+':''}${snap.postMarketChangePct?.toFixed(2)||'0.00'}%)</span>`:''} <span style="font-size:10px;color:var(--text3)">(${snap.marketState||'extended'})</span></div>`:''}
    <div class="metrics-grid">
      ...
      ${snap.ptMean?`<div class="metric-tile" style="grid-column:span 2">
        <div class="metric-label">Analyst Price Target</div>
        <div style="display:flex;align-items:baseline;gap:8px;margin-top:4px">
          <div style="font-family:var(--mono);font-size:20px;font-weight:600;color:${snap.ptMean>snap.price?'var(--green)':'var(--red)'}">$${snap.ptMean.toFixed(2)}</div>
          <div style="font-family:var(--mono);font-size:11px;color:${snap.ptMean>snap.price?'var(--green)':'var(--red)'}">consensus (${snap.ptMean>snap.price?'+':''}${((snap.ptMean-snap.price)/snap.price*100).toFixed(1)}% from current)</div>
        </div>
        <div class="metric-sub" style="margin-top:4px">
          Low: $${snap.ptLow?.toFixed(2)||'N/A'} &nbsp;|&nbsp; High: $${snap.ptHigh?.toFixed(2)||'N/A'}
          ${snap.ptLow&&snap.ptHigh?` &nbsp;|&nbsp; Range: $${(snap.ptHigh-snap.ptLow).toFixed(2)} (${((snap.ptHigh-snap.ptLow)/snap.ptMean*100).toFixed(0)}% spread)`:''}
        </div>
      </div>`:''}
      ${buildR40Tile(snap)}
      ${analystHtml}
...
function buildOptionsTable(){
  ...
  function earningsBanner(){
    ...
    const daysAway=Math.round((earningsD-today)/86400000);
...
async function loadEarningsTab(){
  ...
  for(let i=0;i<watchlist.length;i++){
    const t=watchlist[i];
    try{
      const snap=S.get('snap_'+t);if(!snap?.earningsDate)continue;const today=new Date(),ed=new Date(snap.earningsDate);if(ed<today)continue;
...
function renderEarningsCards(){
  ...
  if(!data.length){const cached=S.get('earnings_data');if(cached?.data){data=cached.data.map(e=>({...e,daysUntil:Math.round((new Date(e.earningsDate)-today)/86400000)})).filter(e=>e.daysUntil>=0);earningsAllData=data;}}
...
async function loadMarketTab(){
  ...
  try{
    ...
    try{[sp500,nasdaq,treasury2y]=await Promise.all([yahooHistory('^GSPC','3mo','1d'),yahooHistory('^IXIC','3mo','1d'),yahooHistory('^USGG2YR','3mo','1d')]);S.set('mkt_sp500',{timestamps:sp500.timestamps.map(d=>d.toISOString()),closes:sp500.closes,ts:mktTs});S.set('mkt_nasdaq',{timestamps:nasdaq.timestamps.map(d=>d.toISOString()),closes:nasdaq.closes,ts:mktTs});S.set('mkt_2y',{timestamps:treasury2y.timestamps.map(d=>d.toISOString()),closes:treasury2y.closes,ts:mktTs});S.set('mkt_ts',mktTs);}
    catch{const cs=S.get('mkt_sp500'),cn=S.get('mkt_nasdaq'),c2=S.get('mkt_2y');if(cs){sp500={timestamps:cs.timestamps.map(d=>new Date(typeof d==='number'?d*1000:d)),closes:cs.closes};isLive=false;mktTs=cs.ts;showOfflineBanner(cs.ts);}if(cn&&!nasdaq)nasdaq={timestamps:cn.timestamps.map(d=>new Date(d)),closes:cn.closes};if(c2&&!treasury2y)treasury2y={timestamps:c2.timestamps.map(d=>new Date(d)),closes:c2.closes};}
    ...
    const spChg=spCurrent&&spPrev?spCurrent-spPrev:null;const spChgPct=spChg&&spPrev?spChg/spPrev*100:null;
    const nqCurrent=nasdaq?.closes?.filter(c=>c!==null).slice(-1)[0]||null;
    const nqPrev=nasdaq?.closes?.filter(c=>c!==null).slice(-2)[0]||null;
    const nqChg=nqCurrent&&nqPrev?nqCurrent-nqPrev:null;const nqChgPct=nqChg&&nqPrev?nqChg/nqPrev*100:null;
    ...
    el.innerHTML=`
      ...
      <!-- T-bill yields -->
      ${(()=>{
        // Fed rate outlook based on 2-year Treasury vs current ^IRX (3-month / near-term fed funds proxy)
        const t2y=treasury2y?.closes?.filter(c=>c!=null);
        const t2yCurrent=t2y&&t2y.length?t2y[t2y.length-1]:null;
        const t2y1mAgo=t2y&&t2y.length>=21?t2y[t2y.length-21]:null;
        if(!t2yCurrent||!tb3Current)return'';
        const spread=t2yCurrent-tb3Current;
        const t2yTrend=t2y1mAgo?(t2yCurrent-t2y1mAgo):null;
        let spreadInterp,spreadColor;
        if(spread<-0.5){spreadInterp='Market pricing in significant rate cuts (2Y well below Fed Funds rate). Tailwind for equity valuations and growth stocks.';spreadColor='var(--green)';}
        else if(spread<-0.1){spreadInterp='Market expecting modest rate cuts. Mild tailwind for equities.';spreadColor='var(--green)';}
        else if(spread<0.1){spreadInterp='Market expects rates roughly unchanged near-term.';spreadColor='var(--text2)';}
        else if(spread<0.5){spreadInterp='Market pricing in modest rate increases. Slight headwind for growth valuations.';spreadColor='var(--warn)';}
        else{spreadInterp='Market pricing in significant rate hikes. Headwind for equity multiples.';spreadColor='var(--red)';}
        const trendStr=t2yTrend!=null?(t2yTrend<-0.1?' -- 2Y falling (cut expectations growing)':t2yTrend>0.1?' -- 2Y rising (fewer cuts expected)':''):'';
        return '<div class="card">'
          +'<div class="card-title"><span class="dot" style="background:var(--accent3)"></span>Fed Rate Outlook</div>'
          +'<div class="metrics-grid">'
          +'<div class="metric-tile"><div class="metric-label">2-Year Treasury (^USGG2YR)</div><div class="metric-value" style="color:var(--accent3)">'+t2yCurrent.toFixed(2)+'%</div><div class="metric-sub">Market\'s implied rate path over next 2 years</div></div>'
          +'<div class="metric-tile"><div class="metric-label">Spread (2Y minus 3M Fed Funds)</div><div class="metric-value" style="color:'+spreadColor+'">'+(spread>=0?'+':'')+spread.toFixed(2)+'%</div><div class="metric-sub">'+(spread<0?'Inverted = cuts expected':'Normal = hikes or hold')+'</div></div>'
          +'</div>'
          +'<div class="commentary">'+spreadInterp+trendStr+'</div>'
          +'</div>';
      })()}
      <div class="card">
...
function scorePuts({price,rsiVal,ma50,ma200,rangePos,earningsDate,recStrike,expiration,estApy,ivrVal}){
  ...
  const earningsScore=!earningsDate?1:(()=>{const d=Math.round((new Date(earningsDate.split(' ')[0])-new Date())/86400000);return d>=0&&d<35?-1:d<60?0:2;})();
...
async function runDashboards(skipOnlineCheck=false){
  ...
  for(let i=0;i<watchlist.length;i++){
    ...
    try{
      ...
      const ps=scorePuts({price,rsiVal,ma50,ma200,rangePos,earningsDate:earningsDisplay,recStrike:pRS,expiration:pExp,estApy:pApy,ivrVal});
      const cs=scoreCalls({price,rsiVal,ma50,ma200,rangePos,earningsDate:earningsDisplay,recStrike:cRS,expiration:cExp,estApy:cApy,ivrVal});
...
async function prefetchAll(){
  ...
  for(let i=0;i<watchlist.length;i++){
    ...
    try{const pt=await fetchPriceTarget(t);if(pt&&pt.ptMean){const sn=S.get('snap_'+t);if(sn){sn.ptMean=pt.ptMean;sn.ptHigh=pt.ptHigh||null;sn.ptLow=pt.ptLow||null;sn.ptAnalysts=pt.ptAnalysts||null;S.set('snap_'+t,sn);}}}catch{}
...
async function refreshSingleTicker(){
  ...
  try{
    ...
    try{const ah=await fetchAfterHoursPrice(t);if(ah){snap.postMarketPrice=ah.postMarketPrice;snap.postMarketChange=ah.postMarketChange||null;snap.postMarketChangePct=ah.postMarketChangePct||null;snap.marketState=ah.marketState;snap.peForward=ah.forwardPE||null;if(ah.trailingEps!=null)snap.epsTTM=ah.trailingEps;
      ...
      try{const pt=await fetchPriceTarget(t);if(pt&&pt.ptMean){snap.ptMean=pt.ptMean;snap.ptHigh=pt.ptHigh||null;snap.ptLow=pt.ptLow||null;snap.ptAnalysts=pt.ptAnalysts||null;}}catch{}
...
function init(){
  ...
  const ce=S.get('earnings_data');if(ce?.data){const today=new Date();earningsAllData=ce.data.map(e=>({...e,daysUntil:Math.round((new Date(e.earningsDate)-today)/86400000)})).filter(e=>e.daysUntil>=0);}
  updateOnlineIndicator();
  // Start market status banner ticker -- every second
  updateMarketBanner();
  setInterval(updateMarketBanner,1000);
  updateHeaderStatus();
